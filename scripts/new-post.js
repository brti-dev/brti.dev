...
console.log('Beginning new post script...')

if (!fileExists(POSTS_DIR)) {
  console.error(
    `Posts dir (${POSTS_DIR}) does not exist or isn't writable. Exiting program...`
  )
  process.exit(0)
}
...
function commandPrompt() {
  ...
  readline.write(
    typeof meta[currentPrompt] === 'string' ||
      meta[currentPrompt] instanceof String
      ? meta[currentPrompt]
      : ''
  )
}
    ...
    switch (i) {
      ...
      case 1:
        ...
        if (fileExists(path.join(POSTS_DIR, `${meta.slug}.md`))) {
          console.error(
            `Error: There's already an article with the slug ${meta.slug}`
          )
        } else {
          i += 1
        }
